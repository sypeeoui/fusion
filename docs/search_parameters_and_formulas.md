...
$$S = S_{\text{board}} \cdot W_{\text{board}} + \frac{A_{\text{path}}}{\text{norm}(d)} \cdot W_{\text{attack}} + \frac{C_{\text{path}}}{\text{norm}(d)} \cdot W_{\text{chain}} + B_{\text{next}} \cdot W_{\text{b2b}} + M_{\text{context}} \cdot W_{\text{context}} - P_{\text{b2b\_break}}$$

Where:
- $S_{\text{board}}$: Structural board evaluation.
- $A_{\text{path}}$: Cumulative attack sent along the path to this node.
- $C_{\text{path}}$: Cumulative shaped chain value along the path.
- $B_{\text{next}}$: Back-to-Back level after the move.
- $M_{\text{context}}$: Contextual modifier based on combo delta and coaching state.
- $P_{\text{b2b\_break}}$: Penalty applied if a B2B chain is broken ($B_{\text{prev}} > 0$ and $B_{\text{next}} = 0$ on a line clear).
- $\text{norm}(d) = \min(\sqrt{d+1}, \text{max\_depth\_factor})$: Normalization factor to prevent long paths from over-valuing cumulative metrics.
